...
const float SHAKE_INTENSITY = 0.00045; // max offset in UV space
const float SHAKE_RATE      = 35.0;    // jumps per second; lower = chunkier
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    if (timeSinceShake >= 0.0 && timeSinceShake < SHAKE_DURATION) {
        // Quadratic decay: hits immediately, settles fast
