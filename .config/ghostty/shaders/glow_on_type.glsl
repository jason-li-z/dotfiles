// Glow (unchanged) + chromatic aberration that only twitches on typing / cursor change.
// Replaces your current glow.glsl. Chain this LAST in your shader list.
...
// High-frequency "twitch" — same idea as the original, but it only matters
// while the type envelope is > 0.
float offsetFunction(float t) {
    float amount = 1.0;
    const float periods[4] = {6.0, 16.0, 19.0, 27.0};
    for (int i = 0; i < 4; i++) {
        amount *= 1.0 + 0.5 * sin(t * periods[i]);
    }
...
// --- typing aberration tuning ---
const float ABBERATION_FACTOR = 0.05;     // px-scale multiplier; lower = subtler split
const float TYPE_AB_DURATION  = 0.18;     // how long the twitch lasts after a keypress
const float TYPE_AB_STRENGTH  = 1.0;      // overall gain on the on-type split

void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    vec2 uv = fragCoord.xy / iResolution.xy;

    // Envelope: 1.0 at the instant the cursor changes, decays to 0 over TYPE_AB_DURATION.
    float sinceType = iTime - iTimeCursorChange;
    float env = 1.0 - clamp(sinceType / TYPE_AB_DURATION, 0.0, 1.0);
    env = env * env; // quadratic falloff — snappy attack, quick settle

    float amount = offsetFunction(iTime) * env * TYPE_AB_STRENGTH;
